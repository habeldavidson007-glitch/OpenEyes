...
def test_version_modes() -> None:
    ...
    payload = _extract_json_payload(machine.output)
...
def test_ask_json_contains_routing_metadata() -> None:
    runner = CliRunner()
    result = runner.invoke(cli, ["--json", "ask", "How does inflation affect bond prices?"])
    assert result.exit_code == 0
